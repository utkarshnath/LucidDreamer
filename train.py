...
def prepare_embeddings_obj(guidance_opt, text, neg_text, guidance):
	embeddings = {}
	# text embeddings (stable-diffusion) and (IF)text
	embeddings['default'] = guidance.get_text_embeds([text])
	embeddings['uncond'] = guidance.get_text_embeds([f'{neg_text}, {guidance_opt.negative}'])
...
def guidance_setup(guidance_opt):
	...
	for text, neg_text in zip(guidance_opt.prompt_obj, guidance_opt.prompt_obj_neg):
		obj_embeddings.append(prepare_embeddings_obj(guidance_opt, text, neg_text, guidance))
	
	for edge, text in guidance_opt.prompt_edge.items():
		print(text)
		edge_embeddings[edge] = prepare_embeddings(guidance_opt, text, guidance)
...
def training(dataset, opt, pipe, gcams, guidance_opt, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, save_video, case):
	first_iter = 0
	...
	for iteration in range(first_iter, opt.iterations + 1):
		...
		if case == 0:
			...
			if num_objs <= 2 or iteration % (num_objs + 1):
				full_scene = False
				obj_loss, _, viewspace_point_tensor_obj, visibility_filter_obj, radii_obj, depth_obj, alpha_obj = forward(opt, 
							embeddings=obj_embeddings[idx], objs=[idx], iteration=iteration, viewpoint_stack=viewpoint_stack,
							scene=scene, guidance_opt=guidance_opt, debug_from=debug_from, gaussians=gaussians, pipe=pipe, 
							background=background, dataset=dataset, guidance=guidance, use_control_net=use_control_net, 
							save_folder=save_folder, gcams=gcams)
				loss = obj_loss
				selected_objs = [[idx]]
				...
				if idx in edges:
					edge = random.choice(edges[idx])
					edge_loss, image, viewspace_point_tensor, visibility_filter, radii, depth, alpha = forward(opt, 
							embeddings=edge_embeddings[str(edge)], objs=edge, iteration=iteration, viewpoint_stack=viewpoint_stack, 
							scene=scene, guidance_opt=guidance_opt, debug_from=debug_from, gaussians=gaussians, pipe=pipe, 
							background=background, dataset=dataset, guidance=guidance, use_control_net=use_control_net, 
							save_folder=save_folder, gcams=gcams)
					wandb.log({"loss/edge_loss": edge_loss.item()})
					loss += edge_loss
					selected_objs = [edge] + selected_objs
					vpt = [viewspace_point_tensor] + vpt
					vf = [visibility_filter] + vf
					radiis = [radii] + radiis	
			else:
				full_scene = True
				# full scene rendering
				loss, image, viewspace_point_tensor, visibility_filter, radii, depth, alpha = forward(opt, 
							embeddings=embeddings, objs=idx_list, iteration=iteration, viewpoint_stack=viewpoint_stack, 
							scene=scene, guidance_opt=guidance_opt, debug_from=debug_from, gaussians=gaussians, pipe=pipe, 
							background=background, dataset=dataset, guidance=guidance, use_control_net=use_control_net, 
							save_folder=save_folder, gcams=gcams)
			...
			if num_objs <= 2 or iteration % (num_objs + 1):
				...
				radiis = [radii_obj]
				wandb.log({"loss/obj_loss": obj_loss.item()})
				if idx in edges and iteration > 4000:
					edge = random.choice(edges[idx])
					edge_loss, image, viewspace_point_tensor, visibility_filter, radii, depth, alpha = forward(opt, 
							embeddings=edge_embeddings[str(edge)], objs=edge, iteration=iteration, viewpoint_stack=viewpoint_stack, 
							scene=scene, guidance_opt=guidance_opt, debug_from=debug_from, gaussians=gaussians, pipe=pipe, 
		...
		with torch.no_grad():
			...
			if (iteration in testing_iterations):
				if save_video:
					video_inference(iteration, scene, render, (pipe, background, idx_list), tb_writer)
...
def video_inference(iteration, scene : Scene, renderFunc, renderArgs, tb_writer):
	...
	if config['cameras'] and len(config['cameras']) > 0:
		...
		wandb.log({"video/rgb_graph": wandb.Video(np.array(img_frames).transpose(0,3,1,2), fps=30)})
...
def video_inference_obj(iteration, scene : Scene, renderFunc, renderArgs, obj, tb_writer):
	...
	if config['cameras'] and len(config['cameras']) > 0:
		...
		for idx, viewpoint in enumerate(config['cameras']):
			...
			#save_image(image,os.path.join(save_folder,"lora_view_{}.jpg".format(viewpoint.uid)))
		# Img to Numpy
		imageio.mimwrite(os.path.join(save_folder, "video_rgb_obj_{}_{}.mp4".format(obj, iteration)), img_frames, fps=30, quality=8)
		if len(depth_frames) > 0:
			imageio.mimwrite(os.path.join(save_folder, "video_depth_obj_{}_{}.mp4".format(obj, iteration)), depth_frames, fps=30, quality=8)
		wandb.log({f"video/rgb_obj_{obj}": wandb.Video(np.array(img_frames).transpose(0,3,1,2), fps=30)})
...
if __name__ == "__main__":
	...
	training(lp, op, pp, gcp, gp, args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.save_video, args.case)
