...
class GaussianModel:
	...
	def create_from_pcd(self, pcd : BasicPointCloud, points_per_obj: list, spatial_lr_scale : float, num_objs: int):
		...
		print("Number of points at initialisation : ", fused_point_cloud.shape[0] * fused_point_cloud.shape[1])

		dist2 = torch.empty((0, ), device='cuda', dtype=torch.float)
		for i in range(self.num_objs):
			dist2 = torch.cat((dist2, torch.clamp_min(distCUDA2(torch.from_numpy(np.asarray(pcd.points[i])).float().cuda()).unsqueeze(0), 0.0000001)), dim=0)
	...
	def save_ply(self, path):
		...
		opacities = self._opacity.reshape(-1, self._opacity.shape[-1]).detach().cpu().numpy()
		scale = self._scaling.reshape(-1, self._scaling.shape[-1]).detach().cpu().numpy()
		rotation = self._rotation.reshape(-1, self._rotation.shape[-1]).detach().cpu().numpy()
		try:
			f_rest = self._features_rest.reshape(-1, self._features_rest.shape[2], self._features_rest.shape[3]).detach().transpose(1, 2).flatten(start_dim=1).contiguous().cpu().numpy()
		except:
			pass
	...
	def reset_opacity(self):
		...
		l = 0
		for i in range(self.num_objs):
			opacities_new.append(inverse_sigmoid(torch.min(self.get_opacity[i, :self.points_per_obj[i]], torch.ones_like(self.get_opacity[i, :self.points_per_obj[i]])*0.01)))
			l = max(l, self.points_per_obj[i])
		opacities_new = self.zero_pad_tensor(opacities_new, l)
		...
		for group in self.optimizer.param_groups:
			...
			if group["name"] not in ['background']:
				if stored_state is not None:
					...
					del self.optimizer.state[group['params'][0]]
					group["params"][0] = nn.Parameter((self.prune_tensor(group["params"][0], mask).requires_grad_(True)))
					self.optimizer.state[group['params'][0]] = stored_state
	...
	def prune_points(self, mask):
		valid_points_mask = []
		for i in range(self.num_objs):
			valid_points_mask.append(~mask[i])
		...
		self.xyz_gradient_accum = self.prune_tensor(self.xyz_gradient_accum, valid_points_mask)

		self.denom = self.prune_tensor(self.denom, valid_points_mask)
	...
	def concatenate_tensors(self, tensor, extension_tensor, for_optim=False):
		...
		for i in range(self.num_objs):
			if for_optim:
				temp.append(torch.cat((tensor[i][:self.points_per_obj[i]], torch.zeros_like(extension_tensor[i])), dim=0))
			else:
		...
		for group in self.optimizer.param_groups:
			if group["name"] not in ['background']:
				...
				if stored_state is not None:
					...
					del self.optimizer.state[group['params'][0]]
					group["params"][0] = nn.Parameter(self.concatenate_tensors(group["params"][0], extension_tensor).requires_grad_(True))
					self.optimizer.state[group['params'][0]] = stored_state
	...
	def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_length):
		d = {"xyz": new_xyz,
		...
		self._rotation = optimizable_tensors["rotation"]
		for i in range(self.num_objs):
			self.points_per_obj[i] += new_length[i]
	...
	def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
		...
		for i in range(self.num_objs):
			n_init_points = self.points_per_obj[i]
			# Extract points that satisfy the gradient condition
			padded_grad = torch.zeros((n_init_points), device="cuda")
			padded_grad[:grads[i][:self.points_per_obj[i]].shape[0]] = grads[i][:self.points_per_obj[i]].squeeze()
			selected_pts_mask = torch.where(padded_grad >= grad_threshold, True, False)
			selected_pts_mask = torch.logical_and(selected_pts_mask,
												torch.max(self.get_scaling[i, :self.points_per_obj[i]], dim=1).values > self.percent_dense*scene_extent)
			temp_selected_masks.append(selected_pts_mask)

			stds = self.get_scaling[i, :self.points_per_obj[i]][selected_pts_mask].repeat(N,1)
			means = torch.zeros((stds.size(0), 3),device="cuda")
			samples = torch.normal(mean=means, std=stds)
			rots = build_rotation(self._rotation[i, :self.points_per_obj[i]][selected_pts_mask]).repeat(N,1,1)
			new_xyz.append(torch.bmm(rots, samples.unsqueeze(-1)).squeeze(-1) + self.get_xyz[i, :self.points_per_obj[i]][selected_pts_mask].repeat(N, 1))
			new_scaling.append(self.scaling_inverse_activation(self.get_scaling[i, :self.points_per_obj[i]][selected_pts_mask].repeat(N,1) / (0.8*N)))
			new_rotation.append(self._rotation[i, :self.points_per_obj[i]][selected_pts_mask].repeat(N,1))
			new_features_dc.append(self._features_dc[i, :self.points_per_obj[i]][selected_pts_mask].repeat(N,1,1))
			new_features_rest.append(self._features_rest[i, :self.points_per_obj[i]][selected_pts_mask].repeat(N,1,1))
			new_opacities.append(self._opacity[i, :self.points_per_obj[i]][selected_pts_mask].repeat(N,1))
			new_points_per_obj.append(new_xyz[i].shape[0])

		self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_points_per_obj)

		prune_filter = []
		for i in range(self.num_objs):
			prune_filter.append(torch.cat((temp_selected_masks[i], torch.zeros(N * temp_selected_masks[i].sum(), device="cuda", dtype=bool))))
		self.prune_points(prune_filter)

		for i in range(self.num_objs):
			self.points_per_obj[i] -= new_points_per_obj[i] // 2
	...
	def densify_and_clone(self, grads, grad_threshold, scene_extent):
		...
		for i in range(self.num_objs):
			...
			new_opacities.append(self._opacity[i, :self.points_per_obj[i]][selected_pts_mask])
			new_scaling.append(self._scaling[i, :self.points_per_obj[i]][selected_pts_mask])
			new_rotation.append(self._rotation[i, :self.points_per_obj[i]][selected_pts_mask])
			new_points_per_obj.append(new_xyz[i].shape[0])
	...
	def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size):
		...
		length = []
		for i in range(self.num_objs):
			prune_mask.append((self.get_opacity[i, :self.points_per_obj[i]] < min_opacity).squeeze())
			if max_screen_size:
				big_points_vs = self.max_radii2D[i, :self.points_per_obj[i]] > max_screen_size
				big_points_ws = self.get_scaling[i, :self.points_per_obj[i]].max(dim=1).values > 0.1 * extent
				prune_mask[i] = torch.logical_or(torch.logical_or(prune_mask[i], big_points_vs), big_points_ws)
			length.append(prune_mask[i].sum())
